...
filled <- terra::focal(chm_rast, w, fun = max, na.rm = T) %>% 
  st_as_stars()

ggplot()+
  geom_stars(data = filled)
```
...
```{r, warning=F, message=F}
las_csf <- classify_ground(jpc_las, algorithm = csf(sloop_smooth = F, class_threshold = 0.06, cloth_resolution = 0.1))

ggplot(data = las@data, aes(x = X, y = Y, color = as.factor(Classification)))+
  geom_point(size = 0.1)+
  scale_color_manual(values = c("forestgreen",
                                "burlywood"))
...
grid_r <- rast_ht %>% 
  ...
  st_as_sf() %>% 
  st_set_crs(value = st_crs(filt)) %>% 
  st_rasterize(dx = 0.01, dy = 0.01)

ggplot()+
  geom_stars(data = rast_ht)
...
```{r}
ggplot()+
  geom_point(data = filt@data, aes(x = X, y = Y, color = factor(Classification)), size = 0.01)+
  coord_equal()
```
...
dsm <- rasterize_canopy(filt, res = 0.1, algorithm = p2r(subcircle = 0.1))
...
ggplot()+
  geom_stars(data = stars_dsm)
```

```{r}
plots_mv <- read_sf("../data/MV plots 2nd.geojson") %>% 
  arrange(plots) %>% 
  st_transform(crs = st_crs(filled))

ggplot()+
  geom_sf(data = plots_mv)
```

```{r}
mean <- st_intersection(plots_mv, st_as_sf(stars_dsm)) %>% 
  arrange(plots) %>% 
  filter(Z > 0.06)
...
ggplot()+
  geom_sf(data = mean, aes(color = type))
...
```{r}
manual <- readxl::read_xlsx("../data/hand collected data/hand data mv jun 21.xlsx") %>% 
  janitor::clean_names() %>% 
  dplyr::select(-sample, -date)
...
```{r}
location <- locate_trees(filt, lmf(ws = 0.25, hmin = 0.06))
...
ggplot()+
  geom_point(data = filt@data, aes(x = X, y = Y, color = factor(Classification)), size = 0.01)+
  geom_sf(data = location, shape = 21, fill = NA)
```
